...
function App() {
  ...
  const handleLogin = (userData: any) => {
    setUser(userData);
    setNotification({ message: `Welcome back, ${userData.name.split(' ')[0]}`, type: 'points' });
  };
  ...
  const NavItem = ({ value, icon: Icon, label }: { value: AppMode, icon: any, label: string }) => {
    ...
    return (
      ...
      >
        ...
        {isActive && (
          <div className="absolute right-3 w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
        )}
      </button>
    );
  };

  // Login Screen Render Check
  if (!user) {
    ...
    <div className="min-h-screen text-slate-200 font-sans selection:bg-emerald-500/30 selection:text-emerald-200 pb-20 md:pb-0 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row min-h-screen">
        ...
        <main className="flex-1 p-4 md:p-8 overflow-x-hidden relative">
          ...
          {notification && (
              <div className={`fixed bottom-24 md:bottom-8 right-8 px-6 py-4 rounded-lg flex items-center gap-4 z-50 animate-in slide-in-from-right duration-500 border bg-[#0B1121]/90 backdrop-blur-md ${
